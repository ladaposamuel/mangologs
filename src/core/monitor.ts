...
export class Monitor {
  ...
  async logRequest(req: RequestData, res: ResponseData, startTime: number, endTime: number): Promise<void> {
    if (!this.config.enabled) return;
    if (this.shouldIgnoreRequest(req.url)) return;
    if (!this.shouldSampleRequest()) return;
