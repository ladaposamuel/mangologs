...
export function parseRequest(req: IncomingMessage | ExpressRequest): ParsedRequest {
  ...
  const url = isExpressRequest ? (req as ExpressRequest).originalUrl || req.url : req.url || '';
...
export function maskSensitiveData(data: any, sensitiveKeys: string[] = ['password', 'token', 'apiKey']): any {
  ...
  for (const [key, value] of Object.entries(data)) {
    if (sensitiveKeys.includes(key.toLowerCase())) {
      ...
      maskedData[key] = maskSensitiveData(value, sensitiveKeys);
    } else {
